...
/**
 * Test ReadAllDrawsCorrectTest
 */
class ReadAllDrawsCorrectTest {
    ...
    void testReadAllDrawsCorrect(String line) throws Exception {
        File file = Files.createTempFile("lotto", ".txt").toFile();
        Model model = new Model(file.getAbsolutePath());

        model.addDraw(line);

        assertFalse(model.readAllDraws().isEmpty());
